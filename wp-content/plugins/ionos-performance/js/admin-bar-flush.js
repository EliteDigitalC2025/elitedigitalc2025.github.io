...
( function () {
	let is_flushing = false,
		admin_bar_ionos_performance_list_item = document.getElementById(
			'wp-admin-bar-ionos-performance'
		),
		flush_link =
			admin_bar_ionos_performance_list_item.querySelector( 'a.ab-item' ),
	...
	const button = document
		...
		.createContextualFragment(
			'<button class="ab-item">' + flush_link.innerHTML + '</button>'
		);
	flush_link.parentNode.replaceChild( button, flush_link );

	const admin_bar_icon = admin_bar_ionos_performance_list_item.querySelector(
		'#wp-admin-bar-ionos-performance .ab-icon'
	);

	document
		.querySelector( '#wp-admin-bar-ionos-performance .ab-item' )
	...
	function flush( event ) {
		...
		is_flushing = true;
		aria_live_area.textContent =
			ionos_performance_admin_bar_flush_ajax_object.flushing;
		...
		request.addEventListener( 'load', function () {
			if ( this.status === 200 ) {
				...
				flush_icon_remove_classes();
				admin_bar_icon.classList.add( 'animate-fade' );
				admin_bar_icon.classList.add(
					ionos_performance_admin_bar_flush_ajax_object.dashicon_success
				);
				aria_live_area.textContent =
					ionos_performance_admin_bar_flush_ajax_object.flushed;
				return;
			}
		...
		request.open(
			'DELETE',
			ionos_performance_admin_bar_flush_ajax_object.url
		);
		request.setRequestHeader(
			'X-WP-Nonce',
			ionos_performance_admin_bar_flush_ajax_object.nonce
		);
		request.send();
	}
} )();
